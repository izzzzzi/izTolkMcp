...
Tolk reduces gas consumption by 30-50% compared to FunC, primarily due to differences in language design and compiler optimizations.
...
FunC requires manual control over preloads and skips. Tolk uses the `lazy` keyword to load only accessed fields.
...
FunC represents only integers: -1 for true, 0 for false. Tolk provides a `bool` type and logical operators `&&`, `||`, and `!`.
...
```tolk
val delta = someSlice.loadUint(32);
val owner = someSlice.loadAddress();
```

### 12. Native Maps Over TVM Dictionaries

FunC uses dictionaries like `m~idict_set_builder(...)`. Tolk provides native maps like `m.set(1, 10)`.
...
FunC allows arbitrary symbols in identifiers. Tolk allows only alphanumeric identifiers.

### 19. Automatic Inlining

In FunC, prefer larger functions for reduced gas consumption. In Tolk, the compiler auto-inlines functions.
...
FunC manually combines constant stores. Tolk merges `b.storeUint(...)` calls if constant.
...
TVM instructions are supported for low-level operations:
...
1. Review the list of differences above
